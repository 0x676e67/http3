...
#[tokio::test]
async fn server_grease_once_across_resolver_entry_points() {
    for send_grease in [false, true] {
        for low_level in [[true, true], [true, false], [false, true]] {
            ...
            let client = async {
                ...
                let fields = [
                    qpack::HeaderField::new(b":method".as_slice(), b"GET".as_slice()),
                    qpack::HeaderField::new(b":scheme".as_slice(), b"https".as_slice()),
                    qpack::HeaderField::new(b":authority".as_slice(), b"localhost".as_slice()),
                    qpack::HeaderField::new(b":path".as_slice(), b"/".as_slice()),
                ];
                let mut block = BytesMut::new();
                qpack::encode_stateless(&mut block, &fields).unwrap();
                let mut request = BytesMut::new();
                Frame::headers(block.freeze()).encode_with_payload(&mut request);
                for index in 0..2 {
                    let (mut send, mut recv) = connection.open_bi().await.unwrap();
                    send.write_all(&request).await.unwrap();
                    send.finish().unwrap();
                    let response = recv.read_to_end(4096).await.unwrap();
                    let mut wire = response.as_slice();
                    let mut headers = 0;
                    let mut grease = 0;
                    while wire.has_remaining() {
                        let kind = VarInt::decode(&mut wire).unwrap().into_inner();
                        let len = usize::try_from(VarInt::decode(&mut wire).unwrap().into_inner())
                            .unwrap();
                        assert!(wire.remaining() >= len);
                        if kind == 1 {
                            headers += 1;
                        } else {
                            // RFC 9114 Section 7.2.8 reserves these frame types for GREASE.
                            assert!(kind >= 0x21);
                            assert_eq!((kind - 0x21) % 0x1f, 0);
                            grease += 1;
                        }
                        wire.advance(len);
                    }
                    assert_eq!(headers, 1);
                    assert_eq!(grease, usize::from(send_grease && index == 0));
                }
                let _ = done_tx.send(());
            };
            let server = async {
                ...
                let mut incoming: server::Connection<_, Bytes> =
                    builder.build(server.next().await).await.unwrap();
                for use_low_level in low_level {
                    let resolver = if use_low_level {
                        let stream = future::poll_fn(|cx| incoming.poll_accept_request_stream(cx))
                            .await
                            .unwrap()
                            .unwrap();
                        incoming.create_resolver(crate::frame::FrameStream::new(
                            crate::stream::BufRecvStream::new(stream),
                        ))
                    } else {
                        incoming.accept().await.unwrap().unwrap()
                    };
                ...
                // Keep the driver alive until the client has consumed both responses.
                let _ = done_rx.await;
            };
