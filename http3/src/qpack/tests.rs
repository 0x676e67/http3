...
fn decoder_update_releases_blocked_stream_budget_before_request_repoll() {
    ...
    let mut cx = Context::from_waker(futures_util::task::noop_waker_ref());
    let mut first_state = DecoderState::new();
    first_state.extend(&mut Cursor::new(first_field_section));
    assert_eq!(
        decoder.poll_decode_header(&mut cx, &mut first_state, true, u64::MAX),
        Poll::Ready(Err(DecoderError::MissingRefs(1)))
    );
    ...
    let mut second_state = DecoderState::new();
    second_state.extend(&mut Cursor::new(second_field_section));
    assert_eq!(
        decoder.poll_decode_header(&mut cx, &mut second_state, true, u64::MAX),
